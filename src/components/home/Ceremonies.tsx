...
export default function Ceremonies() {
  ...
  const [editingCeremony, setEditingCeremony] = useState<Ceremony | null>(
    null
  );
    ...
    return (
      <section id="ceremonias" className="container py-12 md:py-24">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[...Array(3)].map((_, i) => (
             <Card key={i} className="flex flex-col rounded-2xl border-2 border-card-foreground/10 h-[450px] animate-pulse"></Card>
          ))}
        </div>
      </section>
    );
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {ceremonies.map((ceremony, index) => (
          <Card
            key={index}
            className={`relative flex flex-col rounded-2xl border-2 hover:border-primary/80 transition-all duration-300 ${
              ceremony.featured
